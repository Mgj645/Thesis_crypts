...
public class newSchemeV4redis implements SchemeInterface {
    ...
    private final static String usersRedis = "newSchemeUsersa";
    private final static String userNamesRedis = "newSchemeUserNamesa";
    ...
    public boolean login(String username, String password) {
        return jedis.sismember(usersRedis,  applyFunction(username, password));
    }

    public boolean register(String username, String password) {

        if (jedis.sismember(userNamesRedis, username))
            return false;
        else {

            jedis.sadd(usersRedis, applyFunction(username, password));
            jedis.sadd(userNamesRedis, username);
    ...
    public boolean changePassword(String username, String password1, String password2) {
        if(jedis.sismember(usersRedis, applyFunction(username, password1))){
        jedis.sdiff(applyFunction(username, password1));
        jedis.sadd(usersRedis, applyFunction(username, password2));
    ...
    public boolean changeUsername(String username1, String username2, String password) {
        ...
        if (jedis.sismember(usersRedis, applyFunction(username1, password))){
            jedis.sdiff(usersRedis, applyFunction(username1, password));
            jedis.sadd(usersRedis, applyFunction( username2, password));

            jedis.sdiff(userNamesRedis, username1);
            jedis.sadd(userNamesRedis,  username2);
    ...
    public boolean deleteUser(String username, String password) {
        ...
        if (jedis.sismember(usersRedis,  applyFunction(username, password))){
            log.add(new ArrayList<>() {{
                add("del");
                add(username);
                add(password);
            }});
            jedis.sdiff(usersRedis, applyFunction(username, password));
            jedis.sdiff(userNamesRedis, username);
            return true;
        }
            else return false;
    }
