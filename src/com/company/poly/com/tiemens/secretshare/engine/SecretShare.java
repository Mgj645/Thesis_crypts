 ...
 ******************************************************************************
 */
package com.company.poly.com.tiemens.secretshare.engine;
 ...
 *
 * General description: A secret is divided into "n" pieces of data, such that
 * "k" of those pieces can be used to reconstruct the secret, but "k-1" of those
 * pieces gets you nothing.
 *
 * The polynomials generated will be order "k-1", e.g. if k=3, then f(x) =
 * secret + a*x^1 + b*x^2 and there will be "k" of those polynomials, each with
 * random [and discarded] coefficients 'a', 'b', etc.
 *
 * @author tiemens
 *
 */
public class SecretShare {
   ...
   * secret key".
   *
   * Sadly, "larger" is not enough for this implementation [for an unknown
   * reason].
   *
   * Some test data shows: for a secret with 103 bits, 115 is not enough 116
   * is enough for a secret with 159 bits, 160 is enough
   *
   * So - this method provides guidance on the modulus to use for a given
   * secret.
   *
   * @param secret number
   * @return a modulus that (should) work in this library. It may be larger
   * than it needs to be, but it will work.
   */
  ...
  /**
   * NOTE: you should prefer createAppropriateModulusForSecret() over this
   * method.
   *
  ...
  /**
   * Split the secret into pieces, where the caller controls the random
   * instance.
   *
   * @param secret to split
   ...
   * @return the combine output instance [which in turn contains the recovered
   * secret]
   */
  ...
  private void sanityCheckPublicInfos(PublicInfo outer,
    ...
    if (outer.k <= 0) {
      throw new SecretShareException(
              "Public Info [outer] k must be positive, k=" + outer.k);
    }
  ...
  private void sanityCheckShareInfo(final PublicInfo outer,
    ...
    if (outer.k != share.getPublicInfo().k) {
      throw new SecretShareException("Public Info " + indexInfo
              + "mismatch on k, should be = "
    ...
    if (!matches(outer.primeModulus, share.getPublicInfo().primeModulus)) {
      throw new SecretShareException("Public Info " + indexInfo
              + "mismatch on modulus, should be = "
              + outer.primeModulus + " but was = "
              + share.getPublicInfo().primeModulus);
    }
  }
  ...
  /**
   * Holds all the info needed to be a "piece" of the secret. aka a "Share" of
   * the secret.
   *
   * @author tiemens
   *
  ...
  public static class ShareInfo {
    ...
    public boolean equalsType(ShareInfo other) {
      // NOTE: the "public info" does not count toward equality
  ...
  /**
   * When the secret is split, this is the information that is returned. Note:
   * This is NOT the "public" information, since the polynomial used in
   * splitting the secret is in this object. The "public" information is the
   * '.getShareInfos()' method.
   */
  ...
  public ParanoidOutput combineParanoid(List<ShareInfo> shares,
    ...
    if (maximumCombinationsToTest != null) {
      ...
      outputEvery = (combo.getTotalNumberOfCombinations().intValue()
              * percentEvery) / 100 + 1;
    }
    ...
    for (List<SecretShare.ShareInfo> usetheseshares : combo) {
      ...
      if (answer == null) {
        ...
        if (!answer.equals(solve)) {
          throw new SecretShareException(
                  "Paranoid combine failed, on combination at count=" + count);
        }
      }
    }
  ...
  /**
   * Holds the output of the combineParanoid() operation. "Paranoid" is the
   * term used when: "given more shares than needed, check (all) combinations
   * of shares, make sure that _each_ combination of shares returns the same
   * secret"
   *
   */
  public static class ParanoidOutput {
    ...
    public String getParanoidHeaderOutput() {
      String ret = "SecretShare.paranoid(max="
              + ((maximumCombinationsAllowedToTest != null)
                      ? maximumCombinationsAllowedToTest : "all")
              + " combo.total="
